...
const copyJSON = document.querySelector("#copiarJson");
...
function csvToJson(csv) {
    ...
    function copyJson() {
        const jsonCopia = JSON.stringify(json, null, 2);
        navigator.clipboard.writeText(jsonCopia).then(() => {
            alert("JSON copiado para a área de transferência.")
        });
    }
    
    if(json.length > 0) {
        copyJSON.style.display = "flex";
