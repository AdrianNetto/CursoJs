...
function checkAnswer(e) {
    ...
    if(e.target.innerText === questions[currentQuestion].answer) {
        ...
        alert("Resposta coreta!")
    } else {
        wrong++;
        wrongElement.innerText = `Erros: ${wrong}`
        alert(`Errado! A resposta correta era ${questions[currentQuestion].answer}`)
    }
}
...
nextButton.addEventListener("click", () => {
    ...
    if(currentQuestion < questions.length) {
        ...
        alert(`Fim de jogo! Você acertou ${score} e errou ${wrong} de ${questions.length} perguntas.`);
        restartQuiz()
    }
});
